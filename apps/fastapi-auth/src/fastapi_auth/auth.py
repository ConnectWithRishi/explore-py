...
async def validate_token_with_azure(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        expected_issuer_v2 = f"https://login.microsoftonline.com/{settings.azure_tenant_id}/v2.0"
        ...
        expected_issuer = expected_issuer_v1 if actual_issuer == expected_issuer_v1 else expected_issuer_v2
        ...
        try:
            ...
            logger.info(f"Token validated successfully for user: {payload.get('oid', 'unknown')}")
...
async def validate_azure_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    """
...
async def protected_endpoint(token_payload: Dict[str, Any] = Depends(validate_azure_token)) -> AuthResponse:
    """
...
async def get_current_user(token_payload: Dict[str, Any] = Depends(validate_azure_token)) -> UserInfo:
    """
...
@router.get("/roles")
async def get_user_roles(token_payload: Dict[str, Any] = Depends(validate_azure_token)) -> Dict[str, list[str]]:
    """
...
async def get_token_info(token_payload: Dict[str, Any] = Depends(validate_azure_token)) -> Dict[str, Any]:
    """
