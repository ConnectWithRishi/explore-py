...
async def get_obo_token(user_token: str, scopes: list[str]) -> str:
    ...
    try:
        ...
        if "access_token" in result:
            ...
            error_msg = result.get("error_description", "Unknown error during OBO token acquisition")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Graph API call successful for user: {graph_data.get('id', 'unknown')}")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Graph profile API call successful for user: {profile_data.get('id', 'unknown')}")
                ...
                logger.error(f"Graph profile API call failed: {response.status_code} - {response.text}")
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
                    detail=f"Microsoft Graph API error: {response.status_code}"
                )
